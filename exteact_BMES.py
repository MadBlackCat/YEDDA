...
entityRe = r'\[\@.*?\#.*?\*\](?!\#)'
title_list = ["doc_id", "doc_name", "par_id", "par", "label"]
    ...
    if file[-4:] == ".ann":
        ...
        for p, item in enumerate(par_pair):
            if len(item["par"].strip()) > 0:
                result.append([d, file, p, item["par"], item["label"]])
    d = d + 1
pd.DataFrame(result, columns=title_list).to_csv("extract_label/processed_data.csv", header=True)
